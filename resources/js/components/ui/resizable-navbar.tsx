    ...
    return (
        <motion.div
            ...
            className={cn(
                ...
                visible && (isLightText ? 'bg-neutral-900/70' : 'bg-white/40'),
                className,
            )}
        >
            {children}
                ...
                return (
                    ...
                    >
                        {isAnchor ? (
                            ...
                            // 🌐 Jika normal link
                            <Link onClick={onItemClick} className="relative cursor-pointer px-4 py-2" style={{ color: textColor }} href={item.link}>
                                {hovered === idx && (
                                    <motion.div
                                        layoutId="hovered"
                                        className={`absolute inset-0 h-full w-full rounded-full ${isLightText ? 'bg-neutral-800' : 'bg-neutral-100'}`}
        ...
        <AnimatePresence>
            {isOpen && (
                <motion.div
                    ...
                    exit={{ opacity: 0 }}
                    className={cn(
                        'blur(10px) absolute inset-x-0 top-16 z-50 flex w-full flex-col items-start justify-start gap-4 rounded-lg bg-white/95 px-4 py-8 shadow-[0_0_24px_rgba(34,_42,_53,_0.06),_0_1px_1px_rgba(0,_0,_0,_0.05),_0_0_0_1px_rgba(34,_42,_53,_0.04),_0_0_4px_rgba(34,_42,_53,_0.08),_0_16px_68px_rgba(47,_48,_55,_0.05),_0_1px_0_rgba(255,_255,_255,_0.1)_inset]',
                        className,
                    )}
                >
                    {children}
...
export const NavbarLogo = ({ logo }: { logo?: string }) => {
    return (
        <Link href="/" className="relative z-20 mr-4 flex items-center space-x-2 px-2 py-1 text-sm font-normal text-black">
            <img src={logo} alt="SPM" className="h-8 md:h-12" />
        </Link>
    );
};
